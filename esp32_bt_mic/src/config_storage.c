...
{
    if (idx < 0 || idx >= MAX_DEVICES) return ESP_ERR_INVALID_ARG;
    nvs_handle_t h;
    esp_err_t ret = nvs_open(NVS_NAMESPACE, NVS_READWRITE, &h);
    if (ret != ESP_OK) return ret;

    const char *keys[] = {NVS_KEY_DEV_0, NVS_KEY_DEV_1};
    ret = nvs_set_blob(h, keys[idx], addr, ESP_BD_ADDR_LEN);
    if (ret == ESP_OK) {
        ret = nvs_commit(h);
        /* Update dev_count */
        int count = 0;
    ...
    nvs_close(h);
    ESP_LOGI(TAG, "Device %d saved", idx);
    return ret;
}
...
esp_err_t config_storage_load_device(int idx, esp_bd_addr_t addr)
{
    ...
    const char *keys[] = {NVS_KEY_DEV_0, NVS_KEY_DEV_1};
    size_t len = ESP_BD_ADDR_LEN;
    ret = nvs_get_blob(h, keys[idx], addr, &len);
    nvs_close(h);
    return ret;
}
...
int config_storage_get_device_count(void)
{
    ...
    uint8_t count = 0;
    nvs_get_u8(h, NVS_KEY_DEV_COUNT, &count);
    nvs_close(h);
    return (int)count;
}
...
int config_storage_get_active_device(void)
{
    ...
    if (ret != ESP_OK) return 0;
    uint8_t idx = 0;
    nvs_get_u8(h, NVS_KEY_ACTIVE_DEV, &idx);
    nvs_close(h);
    return (int)idx;
}
...
void config_storage_set_active_device(int idx)
{
    ...
    esp_err_t ret = nvs_open(NVS_NAMESPACE, NVS_READWRITE, &h);
    if (ret == ESP_OK) {
        nvs_set_u8(h, NVS_KEY_ACTIVE_DEV, (uint8_t)idx);
        nvs_commit(h);
        nvs_close(h);
    }
}
